...
Below is a Grad-CAM visualization highlighting image regions that most influenced the model’s prediction.
Red/yellow regions indicate higher importance, while blue regions indicate lower influence.

![Grad-CAM Explanation](assets/gradcam_example.png)
...
| Model                     | RMSE        | R² Score |
|---------------------------|-------------|----------|
| Random Forest (Tabular)   | 126,369.70  | 0.873    |
| XGBoost (Multimodal)      | 117,588.97  | 0.890    |

The multimodal XGBoost model achieved **lower RMSE** and **higher R²** compared to the tabular-only Random Forest baseline, demonstrating that incorporating **satellite imagery features improves property price prediction accuracy**.

This represents a ~7% reduction in RMSE and improved variance explanation when visual context from satellite imagery is incorporated.
